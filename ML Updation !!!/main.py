...
async def stock_insights(request: StockInsightRequest) -> dict[str, Any]:
    ...
    df["daily_velocity"] = df["quantity_sold"] / max(
        (
            pd.to_datetime(analysis_date)
            - pd.to_datetime(df["sale_date"].min())
        ).days,
        1,
    )
...
async def customer_risk(request: CustomerRiskRequest) -> dict[str, Any]:
    ...
    risk_probs = model.predict_proba(scaler.transform(df[feature_cols].fillna(0).values))[:, 1]
